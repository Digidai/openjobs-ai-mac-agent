...
const downloadGithubArchive = async (
    ...
    archiveUrlCandidates.push(
      {
        ...
        headers: { 'User-Agent': 'OpenJobs AI Skill Downloader' },
      },
      {
  ...
  archiveUrlCandidates.push({
    ...
    headers: {
      ...
      'User-Agent': 'OpenJobs AI Skill Downloader',
      ...
      if (fs.existsSync(localSource)) {
        ...
        if (stat.isFile()) {
          if (isZipFile(localSource)) {
            const tempRoot = fs.mkdtempSync(path.join(app.getPath('temp'), 'openjobsai-skill-zip-'));
        ...
        const tempRoot = fs.mkdtempSync(path.join(app.getPath('temp'), 'openjobsai-skill-'));
        cleanupPath = tempRoot;
          ...
          if (githubSource) {
            try {
              ...
              if (errno === 'ENOENT' && process.platform === 'win32') {
                throw new Error(
                  'Git executable not found. Please install Git for Windows or reinstall OpenJobs AI with bundled PortableGit.'
            ...
            throw new Error('Git executable not found. Please install Git for Windows or reinstall OpenJobs AI with bundled PortableGit.');
          } else {
            throw error;
          }
        }
