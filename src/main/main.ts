...
const resolveInlineAttachmentDir = (cwd?: string): string => {
  ...
  return path.join(app.getPath('temp'), 'openjobsai', 'attachments');
};
...
const buildLogExportFileName = (): string => {
  ...
  return `openjobsai-logs-${datePart}-${timePart}.zip`;
};
...
if (!gotTheLock) {
  ...
  const initApp = async () => {
    ...
    const defaultProjectDir = path.join(os.homedir(), 'openjobsai', 'project');
